...
def get_saved_networks(_iface):
	...
	if len(r.split("\n")) > 1:
		for network in r.split("\n")[1:-1]:
			...
			networks.append(
				dict(zip(["id", "ssid", "mode"], [nid, parameters[1], int(_get_parameter(_iface, nid, "mode"))])))
	return networks
...
def get_info(_iface):
	...
	name_index = r.find("ESSID:")

	if mode_index != -1 and name_index != -1:
		mode = r[mode_index + 5:].split()[0]
		name = r[name_index + 7:].split("\"")[0]

	return {"mode": mode, "name": name} if name and name else None
